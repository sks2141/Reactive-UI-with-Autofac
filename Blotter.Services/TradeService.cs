...
namespace Blotter.Services
{
    public class TradeService : ITradeService, IDisposable
    {
        ...
        public TradeService(//ILogger logger, 
            TradeGenerator tradeGenerator, ISchedulerProvider schedulerProvider)
        {
            //_logger = logger;
            ...
            return ObservableChangeSet.Create<Trade, long>(cache =>
            {
                ...
                */
                ...
                var expirer = cache
                    ...
                    .Subscribe(x =>
                    {
                        //_logger.Info("{0} filled trades have been removed from memory", x.Count());
                    });
